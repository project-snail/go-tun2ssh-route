...
func NewUDPDirectHandler(timeout time.Duration) core.UDPConnHandler {
	return &udpHandler{
		timeout:        timeout,
...
func (h *udpHandler) Connect(conn core.UDPConn, target *net.UDPAddr) error {
	...
	log.Infof("new direct connection for target: %s:%s", target.Network(), target.String())
	return nil
}
...
func (h *udpHandler) ReceiveTo(conn core.UDPConn, data []byte, addr *net.UDPAddr) error {
	...
	if ok1 && ok2 {
		...
		return errors.New(fmt.Sprintf("direct connection %v->%v does not exists", conn.LocalAddr(), addr))
	}
}
